...
<aside class="app-sidebar">
    <div class="app-sidebar__user">
        <div>
            <p class="app-sidebar__user-name text-success">{{ auth()->user()->name }}</p>
            <p class="app-sidebar__user-designation">{{ implode(', ',auth()->user()->roles->pluck('name')->toArray())}}</p>
        </div>
    </div>
    <ul class="app-menu">

        <li><a class="app-menu__item " href="{{route('dashboard.home')}}"><i class="app-menu__icon fa fa-dashboard"></i><span class="app-menu__label">Dashboard</span></a></li>

        <!-- categories -->
         @if(auth()->user()->hasPermission('categories_read'))

            <li><a class="app-menu__item " href="{{route('dashboard.categories.index')}}"><i class="app-menu__icon fa fa-bars"></i><span class="app-menu__label">Categories</span></a></li>
        @endif

        <!-- Movie -->
        @if(auth()->user()->hasPermission('movies_read'))

            <li><a class="app-menu__item " href="{{route('dashboard.movies.index')}}"><i class="app-menu__icon fa fa-youtube-play"></i><span class="app-menu__label">Movies</span></a></li>
        @endif

       <!-- roles -->
         @if(auth()->user()->hasPermission('roles_read'))

            <li><a class="app-menu__item " href="{{route('dashboard.roles.index')}}"><i class="app-menu__icon fa fa-unlock-alt"></i><span class="app-menu__label">Roles</span></a></li>
